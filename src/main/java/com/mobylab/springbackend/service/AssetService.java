...
public class AssetService {
    ...
    public AssetDto getAssetBySymbol(String symbol) {
        Asset asset = assetRepository.findBySymbol(symbol)
                .orElseThrow(() -> new BadRequestException("Asset with symbol '" + symbol + "' does not exist."));
        return toDto(asset);
    }

    public List<AssetDto> getAssetByPrefix(String prefix) {
        List<Asset> assetList = assetRepository.findBySymbolStartingWith(prefix).orElseThrow(() -> new BadRequestException("There are no assets with prefix '" + prefix + "'"));
    ...
    public AssetDto updateAssetPrice(AssetPriceUpdateDto assetPriceUpdateDto) {
        Asset asset = assetRepository.findBySymbol(assetPriceUpdateDto.getSymbol())
                .orElseThrow(() -> new BadRequestException("Asset with symbol '" + assetPriceUpdateDto.getSymbol() + "' does not exist."));
    ...
    public AssetDto updateAsset(Long id, AssetCreateDto dto) {
        ...
        Asset asset = assetRepository.findById(id)
                .orElseThrow(() -> new BadRequestException("Asset with id '" + id + "' does not exist."));
